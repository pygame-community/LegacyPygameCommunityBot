...
async def on_ready():
    ...
    """
    await pgbot.init()
    await common.task_manager.dispatch_client_event(events.OnReady(_client=bot))
...
async def on_member_update(before: discord.Member, after: discord.Member):
    await common.task_manager.dispatch_client_event(
        events.OnMemberUpdate(before, after, _client=bot)
    )
...
async def on_member_ban(
    ...
    await common.task_manager.dispatch_client_event(
        events.OnMemberBan(guild, user, _client=bot)
    )
...
async def on_member_unban(guild: discord.Guild, user: discord.User):
    await common.task_manager.dispatch_client_event(
        events.OnMemberUnban(guild, user, _client=bot)
    )
...
async def on_message_edit(before: discord.Message, after: discord.Message):
    ...
    await common.task_manager.dispatch_client_event(
        events.OnMessageEdit(
            ...
            after,
            _client=bot,
            _timestamp=after.edited_at.astimezone(datetime.timezone.utc),
        )
    )
...
async def on_reaction_clear(
    ...
    await common.task_manager.dispatch_client_event(
        events.OnReactionClear(message, reactions, _client=bot)
    )
...
async def on_raw_reaction_clear(payload: discord.RawReactionClearEvent):
    await common.task_manager.dispatch_client_event(
        events.OnRawReactionClear(payload, _client=bot)
    )
...
async def on_reaction_clear_emoji(reaction: discord.Reaction):
    await common.task_manager.dispatch_client_event(
        events.OnReactionClearEmoji(reaction, _client=bot)
    )
...
async def on_raw_reaction_clear_emoji(payload: discord.RawReactionClearEmojiEvent):
    await common.task_manager.dispatch_client_event(
        events.OnRawReactionClearEmoji(payload, _client=bot)
    )
...
async def on_private_channel_create(channel: discord.abc.PrivateChannel):
    await common.task_manager.dispatch_client_event(
        events.OnPrivateChannelCreate(channel, _client=bot)
    )
...
async def on_private_channel_delete(channel: discord.abc.PrivateChannel):
    await common.task_manager.dispatch_client_event(
        events.OnPrivateChannelDelete(channel, _client=bot)
    )
...
async def on_private_channel_update(
    ...
    await common.task_manager.dispatch_client_event(
        events.OnPrivateChannelUpdate(before, after, _client=bot)
    )
...
async def on_private_channel_pins_update(
    ...
    await common.task_manager.dispatch_client_event(
        events.OnPrivateChannelPinsUpdate(channel, last_pin, _client=bot)
    )
...
async def on_guild_channel_create(channel: discord.abc.GuildChannel):
    await common.task_manager.dispatch_client_event(
        events.OnGuildChannelCreate(channel, _client=bot)
    )
...
async def on_guild_channel_delete(channel: discord.abc.GuildChannel):
    await common.task_manager.dispatch_client_event(
        events.OnGuildChannelDelete(channel, _client=bot)
    )
...
async def on_guild_channel_update(
    ...
    await common.task_manager.dispatch_client_event(
        events.OnGuildChannelUpdate(before, after, _client=bot)
    )
...
async def on_guild_channel_pins_update(
    ...
    await common.task_manager.dispatch_client_event(
        events.OnGuildChannelPinsUpdate(channel, last_pin, _client=bot)
    )
...
async def on_guild_integrations_update(guild: discord.Guild):
    await common.task_manager.dispatch_client_event(
        events.OnGuildIntegrationsUpdate(guild, _client=bot)
    )
...
async def on_webhooks_update(channel: discord.abc.GuildChannel):
    await common.task_manager.dispatch_client_event(
        events.OnWebhooksUpdate(channel, _client=bot)
    )
...
async def on_user_update(before: discord.User, after: discord.User):
    await common.task_manager.dispatch_client_event(
        events.OnUserUpdate(before, after, _client=bot)
    )
...
async def on_guild_join(guild: discord.Guild):
    await common.task_manager.dispatch_client_event(
        events.OnGuildJoin(guild, _client=bot)
    )
...
async def on_guild_remove(guild: discord.Guild):
    await common.task_manager.dispatch_client_event(
        events.OnGuildRemove(guild, _client=bot)
    )
...
async def on_guild_update(before: discord.Guild, after: discord.Guild):
    await common.task_manager.dispatch_client_event(
        events.OnGuildUpdate(before, after, _client=bot)
    )
...
async def on_guild_role_create(role: discord.Role):
    await common.task_manager.dispatch_client_event(
        events.OnGuildRoleCreate(role, _client=bot)
    )
...
async def on_guild_role_delete(role: discord.Role):
    await common.task_manager.dispatch_client_event(
        events.OnGuildRoleDelete(role, _client=bot)
    )
...
async def on_guild_role_update(before: discord.Role, after: discord.Role):
    await common.task_manager.dispatch_client_event(
        events.OnGuildRoleUpdate(before, after, _client=bot)
    )
...
async def on_guild_emojis_update(
    ...
    await common.task_manager.dispatch_client_event(
        events.OnGuildEmojisUpdate(guild, before, after, _client=bot)
    )
...
async def on_guild_available(guild: discord.Guild):
    await common.task_manager.dispatch_client_event(
        events.OnGuildAvailable(guild, _client=bot)
    )
...
async def on_guild_unavailable(guild: discord.Guild):
    await common.task_manager.dispatch_client_event(
        events.OnGuildUnavailable(guild, _client=bot)
    )
...
async def on_voice_state_update(
    ...
    await common.task_manager.dispatch_client_event(
        events.OnVoiceStateUpdate(member, before, after, _client=bot)
    )
...
async def on_invite_create(invite: discord.Invite):
    await common.task_manager.dispatch_client_event(
        events.OnInviteCreate(invite, _client=bot)
    )
...
async def on_invite_delete(invite: discord.Invite):
    await common.task_manager.dispatch_client_event(
        events.OnInviteDelete(invite, _client=bot)
    )
...
async def on_group_join(channel: discord.GroupChannel, user: discord.User):
    await common.task_manager.dispatch_client_event(
        events.OnGroupJoin(channel, user, _client=bot)
    )
...
async def on_group_remove(channel: discord.GroupChannel, user: discord.User):
    await common.task_manager.dispatch_client_event(
        events.OnGroupRemove(channel, user, _client=bot)
    )
...
async def on_relationship_add(relationship: discord.Relationship):
    await common.task_manager.dispatch_client_event(
        events.OnRelationshipAdd(relationship, _client=bot)
    )
...
async def on_relationship_remove(relationship: discord.Relationship):
    await common.task_manager.dispatch_client_event(
        events.OnRelationshipRemove(relationship, _client=bot)
    )
...
async def on_relationship_update(
    ...
    await common.task_manager.dispatch_client_event(
        events.OnRelationshipUpdate(before, after, _client=bot)
    )
