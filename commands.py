	...
	if i(args, 0) == 'doc' and len(args) == 2:
		...
		if not returned.exc:
			...
			if len(str_repr) > 2048:
				await sendEmbed(msg.channel, 'Returned text (executed in {duration} sec(s)):', str_repr[:2044] + ' ...')
			else:
				await sendEmbed(msg.channel, 'Returned text (executed in {duration} sec(s)):', str_repr)
